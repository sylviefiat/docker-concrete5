...
class PageList extends DatabaseItemList implements PermissionableListItemInterface
{
    ...
    /**
     * Filters by topics. Doesn't look at specific attributes –instead, actually joins to the topics table.
     */
    public function filterByTopics($topics)
    {
	$this->query->innerJoin('cv', 'CollectionAttributeValues', 'cavTopics', 'cv.cID = cavTopics.cID and cv.cvID = cavTopics.cvID');
        $this->query->innerJoin('cavTopics', 'atSelectedTopics', 'atst', 'cavTopics.avID = atst.avID');
	...
	$db = \Database::get();
		...
		if (is_object($topic)) {
                    array_push($treeNodeIDs,$topic->getTreeNodeID());
                } else {
                    array_push($treeNodeIDs,$topic);
                }
	    }
	    $this->query->andWhere(
                $this->query->expr()->in('atst.TopicNodeID', array_map(array($db, 'quote'), $treeNodeIDs))
            );	
	} else {
            if (is_object($topic)) {
                $treeNodeID = $topic->getTreeNodeID();
